...
export default function initXmpp(mainApp, server) {
  const wss = new WebSocketServer({ server });
  
  mainApp.get("/xmpp/clients", (req, res) => {
  ...
  mainApp.get("/xmpp/client-names", (req, res) => {
    res.send(global.Clients.map((i) => i.displayName).join(", "));
  });

  wss.on("listening", () => {
    logger.xmpp("XMPP WebSocket Server running on the main port");
  });
